...
# Structure: {chat_id: {'mp3_link': str, 'title': str, 'timestamp': float}}
...
def get_youtube_title(video_id):
    """Get video title from video ID (optional, can be enhanced)"""
    # This is a simple fallback - you could use a different API for titles
    return f"YouTube Video {video_id}"

def get_mp3_link(video_id, timeout=30):
    """
    Get MP3 download link from youtube-mp36.p.rapidapi.com
    """
    try:
        logger.info(f"🎵 Getting MP3 link for video ID: {video_id}")
        ...
        title = data.get('title', get_youtube_title(video_id))
        ...
        try:
            head_response = requests.head(mp3_link, timeout=10)
            if head_response.status_code != 200:
                logger.warning(f"MP3 link may be invalid: {head_response.status_code}")
        except:
            pass
...
def send_mp3_link(chat_id, mp3_link, title=""):
    """Send MP3 link to user"""
    try:
        # Send as a document link or audio file
        message = (
            f"🎵 **MP3 របស់អ្នកបានត្រៀមរួចរាល់!**\n\n"
            f"📝 **ចំណងជើង:** {title}\n\n"
            f"🔗 **តំណទាញយក:**\n{mp3_link}\n\n"
            f"⚠️ តំណនេះអាចផុតកំណត់ក្នុងរយៈពេលខ្លី"
        )
        ...
        logger.error(f"Failed to send MP3 link: {e}")
        return False
...
class MP3ConversionWorker:
    ...
    def _process_task(self, task):
        ...
        try:
            # Get MP3 link with timeout
            result = get_mp3_link(video_id, timeout=30)
            ...
            if result['success']:
                ...
                bot.edit_message_text(
                    text=(
                        ...
                        f"សូមចុចមើល Ads ១០ វិនាទី ដើម្បីទទួលបាន Link ទាញយក។"
                    ),
                    chat_id=chat_id,
...
def send_welcome(message):
    ...
    welcome_text = (
        "🎵 សូមស្វាគមន៍មកកាន់ YouTube to MP3 Downloader Bot!\n\n"
        "📱 របៀបប្រើប្រាស់:\n"
        "1️⃣ ផ្ញើតំណភ្ជាប់ YouTube មកកាន់ bot\n"
        "2️⃣ រង់ចាំបន្តិច (ប្រព័ន្ធកំពុងបំប្លែង)\n"
        "3️⃣ ចុចប៊ូតុង 👁️ មើល Ads\n"
        "4️⃣ រង់ចាំ ១០ វិនាទី\n"
        "5️⃣ ចុច 🎵 ទាញយក MP3\n\n"
        "✨ លក្ខណៈពិសេស:\n"
        "• បំប្លែង YouTube ទៅជា MP3\n"
        "• គុណភាពសំឡេងខ្ពស់\n"
        "• ដំណើរការលឿន\n"
        "• ប្រើដោយឥតគិតថ្លៃ\n\n"
        "🔗 ឧទាហរណ៍តំណភ្ជាប់:\n"
...
def handle_download_mp3(call):
    ...
    try:
        ...
        # Sufficient time passed - send the MP3 link
        bot.answer_callback_query(
            call.id,
            text="កំពុងផ្ញើតំណ MP3... សូមរង់ចាំ",
            show_alert=False
        )
        
        # Send MP3 link
        success = send_mp3_link(
            chat_id, 
            mp3_data['mp3_link'], 
            mp3_data.get('title', 'YouTube MP3')
        )
        ...
        if success:
            ...
            bot.edit_message_text(
                text=(
                    ...
                    f"📝 ចំណងជើង: {mp3_data.get('title', 'YouTube Video')}\n\n"
            ...
            bot.answer_callback_query(
                call.id,
                text="❌ បរាជ័យក្នុងការផ្ញើតំណ។ សូមព្យាយាមម្តងទៀត។",
                show_alert=True
            )
...
if __name__ == '__main__':
    ...
    logger.info("✅ Dedicated YouTube to MP3 mode enabled")
